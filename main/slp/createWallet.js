...
export async function createWallet (name) {
    ...
    outStr += '\n\n\n'
    fs.writeFile(`../wallets/${name}-wallet-info.txt`, outStr, function (err) {
        if (err) return console.error(err)

        console.log(`${name}-wallet-info.txt written successfully.`)
    })

    // Write out the basic information into a json file for other apps to use.
    fs.writeFile(`../wallets/${name}-wallet.json`, JSON.stringify(outObj, null, 2), function (err) {
        if (err) return console.error(err)
        console.log(`${name}-wallet.json written successfully.`)
    })
}
